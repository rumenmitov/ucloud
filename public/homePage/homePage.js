...
if (!localStorage.getItem('ucloud'))
  localStorage.setItem('ucloud', JSON.stringify({ username: username }));
let currentDir = pwd;
let freeSpace;
...
function getAllFiles(dir) {
  ...
  xhttp.onload = function () {
    ...
    avatar.addEventListener('click', function () {
      if (responseOBJ.greeter !== 'You are a GUEST')
...
function displayFiles(responseArray) {
  ...
  responseArray.forEach((file) => {
    ...
    const type = file.type ? file.type.toLowerCase() : null;
...
function renameFile(event) {
  ...
  if (type == 'directory') {
    if (newFileName.split('.')[1])
      alert(
        'Since this is a directory, everything after the first dot will be dropped'
      );
...
function deleteFile(event) {
  ...
  let confirmation = confirm(
    `Are you sure you want to delete '${event.target.id}'?`
  );
...
window.onload = () => {
  ...
  domOBJ.userFile.onchange = function () {
    if (this.files[0].size > freeSpace) {
      alert(
        'Upload unsuccessful! You do not have enough space on your account!'
      );
      this.value = '';
    }
  };
