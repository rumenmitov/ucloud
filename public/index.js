...
function displayFiles(responseArray) {
    ...
    responseArray.forEach(file=>{
        ...
        const link = `https://192.168.178.86/${file.linkUrl}`;
        ...
        if (!type) {
            ...
            image.onclick = (e) => {
                location.href = `https://192.168.178.86?pwd=${file.name}`;
            };
            ...
            if (type === 'jpg' || type === 'png' || type === 'webp' || type === 'gif') {
                ...
                image.id = prevDir;
                image.onclick = (e) => {
                    location.href = `https://192.168.178.86?pwd=${prevDirPath}`;
                };
                fileBox.appendChild(image);
...
function deleteFile(event) {
    ...
    if (confirmation) {
        ...
        deleteRequest.onload = function() {
            ...
            location.href = `https://192.168.178.86?pwd=${currentDir}`;
        }
    }
}
