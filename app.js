        ...
        usersCollection
          ...
          .toArray((err, results) => {
            ...
            usersCollection.insertOne(req.body, (err) => {
              ...
              fs.mkdir(
                __dirname + "/users/" + req.body.username,
                (err) => {
                  if (err) console.log(err);
...
userExistsRouter.route('/').post((req, res)=>{
  client.connect(err =>{
    ...
    usersCollection.find({ 'username': req.body.user }).toArray((err, results)=>{
      ...
      folderSize(__dirname + `/users/${req.body.user}`, (err, bytes)=>{
        if (err) console.log(err);
...
homeRouter.route("/").post((req, res) => {
  ...
  let fileListPath = __dirname + "/users/" + dir + "/ucloud_files.txt";
  ...
  if (fs.existsSync(__dirname + `/users/${req.session.username}/.${req.session.username}/ucloud_greeter.txt`, { encoding: 'utf-8' })) {
    greeterText = fs.readFileSync(__dirname + `/users/${req.session.username}/.${req.session.username}/ucloud_greeter.txt`, { encoding: 'utf-8' });
  }

  let avatarPath = '../images_website/avatar.png';
   if (fs.existsSync(__dirname + `/users/${req.session.username}/.${req.session.username}/${req.session.username}_avatar.png`)) {
...
avatarRouter.route('/').post((req, res, next)=>{
  ...
  form.parse(req, (err, fields, files)=>{
    ...
    fs.rename(files.avatarImg.filepath, __dirname + `/users/${req.session.username}/.${req.session.username}/${req.session.username}_avatar.png`, err=>{
      if (err) console.log(err);
...
mkdirRouter.route("/").post((req, res, next) => {
  ...
  fs.mkdir(__dirname + "/users/" + dir + "/" + name, (err) => {
    if (err) console.log(err);

    fs.writeFileSync(
      __dirname + "/users/" + dir + "/" + name + "/ucloud_files.txt",
      "back.backlink",
      { encoding: "utf-8", flag: "w" }
    );
...
searchRouter.route("/:searchOBJ").get((req, res) => {
  ...
  find.file(
    query,
    __dirname + "/users/" + searchOBJ.path,
    function (searchResults) {
      searchResults.forEach((file) => {
        ...
        let filePath = path
          .relative(__dirname + "/users/" + searchOBJ.path, file)
          .split("\\")
          .join("/");
      ...
      find.dir(query, __dirname + "/users/" + searchOBJ.path, function (searchResults) {
        searchResults.forEach((file) => {
    ...
    usersCollection.find({ username: search_query }).toArray((err, results) => {
      ...
      if (results[0]) {
        results.forEach(user => {
          if (fs.existsSync(__dirname + `/users/${user.username}/.${user.username}/${user.username}_avatar.png`)) user.avatarLink = `../users/${user.username}/.${user.username}/${user.username}_avatar.png`;
...
renameRouter.route("/:renameOBJ").put((req, res, next) => {
  ...
  let fileListPath =
    __dirname + "/users" + prevDirAbs + "/ucloud_files.txt";
  ...
  if (dir[0] != "/") dirAbs = "/" + dir;
  fs.rename(
    __dirname + "/users" + dirAbs,
    __dirname + "/users" + prevDirAbs + "/" + queryOBJ.newFileName,
    (err) => {
      if (err) console.log(err);
    }
  );
...
deleteRouter.route("/:deleteOBJ").delete((req, res, next) => {
  ...
  let fileListPath =
    __dirname + "/users" + prevDir + "/ucloud_files.txt";

  let allFilesArray = fs.readFileSync(fileListPath, "utf-8").split("\r\n");

  if (dir[0] != "/") dir = "/" + dir;
  fs.rm(__dirname + "/users" + dir, { recursive: true }, (err) => {
    if (err) console.log(err);
  });
