...
signupRouter.route('/').post((req, res, next) => {
  ...
  if (req.body.password !== req.body.password_confirm) {
    res.send(
      `Passwords do not match! <a href='https://${LINK}/signup/signup.html?userCode=${Buffer.from(
        req.body.email
      ).toString('base64')}'>Try again!</a>`
    );
    return next();
  }
...
homeRouter.route('/').post((req, res) => {
  ...
  if (
    fs.existsSync(
      __dirname +
        `/public/users/${req.session.username}/.${req.session.username}/ucloud_greeter.txt`,
      { encoding: 'utf-8' }
    )
  ) {
    greeterText = fs.readFileSync(
      __dirname +
        `/public/users/${req.session.username}/.${req.session.username}/ucloud_greeter.txt`,
      { encoding: 'utf-8' }
    );
...
avatarRouter.route('/').post((req, res, next) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (
      ...
      path.extname(files.avatarImg.originalFilename) !== '.png' &&
      path.extname(files.avatarImg.originalFilename) !== '.gif' &&
      path.extname(files.avatarImg.originalFilename) !== '.webp'
    ) {
      res.send(
        `Profile pic must be one of the following file types: jpeg, png, gif, webp. <a href='https://${LINK}/homePage/homePage.html?pwd=${req.session.username}'>Back to site</a>`
      );
      return next();
    }

    fs.rename(
      files.avatarImg.filepath,
      __dirname +
        `/public/users/${req.session.username}/.${req.session.username}/${req.session.username}_avatar.png`,
      (err) => {
        if (err) { console.log(err); }
...
greeterRouter.route('/').post((req, res, next) => {
  ...
  res.send(
    `Greeter changed successfully! <a href='https://${LINK}/homePage/homePage.html?pwd=${req.session.username}'>Back to site</a>`
  );
});
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (Array.isArray(files.userFile)) {
      for (let i = 0; i < files.userFile.length; i++) {
        ...
        if (
          files.userFile[i].originalFilename.toLowerCase() ===
          'ucloud_files.txt'
        ) {
          res.send(
            `File cannot be named: <i>${files.userFile[
              i
            ].originalFilename.toLowerCase()}</i>! Please try a different name: <a href='https://${LINK}/homePage/homePage.html?pwd=${
              fields.filePath
            }'>Back to site</a>`
          );
          return next();
      ...
      if (
        files.userFile.originalFilename.toLowerCase() === 'ucloud_files.txt'
      ) {
        res.send(
...
let serverBackend = express()
  ...
  .use('/changeGreeter', greeterRouter)
